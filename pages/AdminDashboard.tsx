...
import { Transaction, User, UserRole, AuditLog } from '../types';
...
// -----------------------------
// Overview Tab (UPDATED: includes AI stats)
// -----------------------------

const OverviewTab: React.FC<{ transactions: Transaction[] }> = ({ transactions }) => {
  const stats = useMemo(() => {
    ...
    // ✅ AI score support: handle both modelScore and mlProbability
    const getScore = (t: Transaction): number | null => {
      const anyT = t as any;
    ...
    const avgScore = scoredCount > 0 ? scored.reduce((a, b) => a + b, 0) / scoredCount : null;
  ...
  const avgScoreText =
    stats.avgScore !== null ? `${(stats.avgScore * 100).toFixed(1)}%` : 'N/A';

  const scoredSubtitle =
    stats.scoredCount > 0
      ? `${stats.scoredCount} scored txn(s)`
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Stats Grid (UPDATED: includes AI cards) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          color="#10B981"
          icon={
            <svg className="h-6 w-6" style={{ color: '#10B981' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-red-100 mt-1">
              {stats.fraudulent} fraudulent out of {stats.total} total transactions
            </p>
          </div>
...
const UserManagementTab: React.FC<{
  ...
  const handleRoleChange = async (username: string, newRole: UserRole) => {
    ...
    try {
      ...
      setError(`Failed to update role: ${err?.message || 'Unknown error occurred'}`);
  ...
  const handleBanToggle = async (username: string, currentBanStatus: boolean) => {
    ...
    try {
      ...
      setError(`Failed to update ban status: ${err?.message || 'Unknown error occurred'}`);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-100 dark:border-gray-700 space-y-4">
      ...
      {error && (
        <div className="mb-2 p-3 bg-red-50 border border-red-200 text-red-700 rounded-md flex justify-between items-center text-sm">
          <span>{error}</span>
          <button onClick={() => setError(null)} className="font-bold hover:text-red-900">
            ×
          </button>
        </div>
      )}
            ...
            {users.map(u => (
              <tr
                ...
                className={`hover:bg-gray-50 dark:hover:bg-gray-800/70 transition-colors ${
                  loadingUser === u.username ? 'opacity-60' : ''
                }`}
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <select
                    ...
                    disabled={u.username === currentUser?.username || loadingUser === u.username}
                  >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                  <button
                    ...
                    disabled={u.username === currentUser?.username || loadingUser === u.username}
                  >
                    {loadingUser === u.username ? '...' : u.isBanned ? 'Unban' : 'Ban'}
                  </button>
                </td>
              </tr>
            ))}
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {logs.map(log => (
              <tr key={log.id}>
                ...
                <td className="px-4 py-2 text-gray-700 dark:text-gray-200">{log.details}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
type AdminTab = 'overview' | 'transactions' | 'users' | 'logs';
...
const AdminDashboard: React.FC<AdminDashboardProps> = props => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'transactions':
        // Uses EmployeeDashboard, which already shows AI modelScore
        return (
          <EmployeeDashboard
