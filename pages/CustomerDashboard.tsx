...
const NewTransactionTab: React.FC<{
    ...
    return (
        <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">Enter transaction details to analyze for potential fraud.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
                 <div>
                    ...
                    <input type="number" id="amount" value={amount} onChange={e => setAmount(e.target.value)}
                        ...
                        placeholder="e.g., 49.99" required min="0.01" step="0.01" disabled={isBanned} />
                </div>
                <div>
                    ...
                    <input type="text" id="merchant" value={merchant} onChange={e => setMerchant(e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                        placeholder="e.g., Amazon, Starbucks" required disabled={isBanned}/>
                </div>
                <div>
                    <label htmlFor="location" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Location</label>
                    <input type="text" id="location" value={location} onChange={e => setLocation(e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                        placeholder="e.g., New York, NY" required disabled={isBanned} />
                </div>
                <button type="submit" disabled={isLoading || isCardFrozen || isBanned}
                    ...
                    {isBanned ? 'Account Suspended' : isCardFrozen ? 'Card is Frozen' : isLoading ? 'Analyzing...' : 'Check Transaction'}
                </button>
            </form>
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                        {filteredTransactions.map(t => (
                            <tr key={t.id}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{new Date(t.timestamp).toLocaleString()}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{t.merchant}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${t.amount.toFixed(2)}</td>
                    ...
                    <p className="text-center py-8 text-gray-500 dark:text-gray-400">No transactions match your filters.</p>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-8">
            <div>
                 ...
                 <p className="text-4xl font-bold text-gray-900 dark:text-white">${user.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                        <span className="font-medium text-gray-800 dark:text-gray-200">Freeze Card</span>
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" checked={user.cardFrozen} onChange={e => onUpdateUser({ cardFrozen: e.target.checked })} className="sr-only peer" disabled={user.isBanned} />
                    ...
                    {user.cardFrozen && <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-2">Your card is frozen. All transactions will be declined.</p>}
                </div>

                <div>
                    <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Custom Alerts</h3>
                    <div className="flex items-center gap-4">
                        <input type="number" value={threshold} onChange={e => setThreshold(e.target.value)} placeholder="e.g., 500"
                            className="flex-grow block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                            disabled={user.isBanned}/>
                        <button onClick={handleSetThreshold} className="py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-300" disabled={user.isBanned}>Set Threshold</button>
                    </div>
                     <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Leave blank to disable. You'll get an alert for any transaction over this amount.</p>
                </div>
            </div>
            ...
            <div>
                <h3 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">Transfer Funds</h3>
                 <form onSubmit={handleTransfer} className="space-y-4">
                    <div>
                        <label htmlFor="recipient" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Recipient's Email</label>
                     ...
                     <div>
                        ...
                        <input type="number" id="transfer-amount" value={transferAmount} onChange={e => setTransferAmount(e.target.value)} required min="0.01" step="0.01"
                            className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                             disabled={user.isBanned}/>
                    </div>
                    <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-indigo-300" disabled={user.isBanned}>Send Money</button>
                 </form>
                 {transferStatus && <p className={`mt-2 text-sm ${transferStatus.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>{transferStatus.message}</p>}
            </div>
        </div>
    );
};
...
interface CustomerDashboardProps {
    ...
    onAddTransaction: (transaction: Omit<Transaction, 'id' | 'userEmail' | 'status' | 'reason' | 'timestamp'>) => void;
    onUpdateUser: (data: Partial<User>) => void;
    onTransferFunds: (recipientEmail: string, amount: number) => Promise<void>;
    isLoading: boolean;
    error: string | null;
...
const CustomerDashboard: React.FC<CustomerDashboardProps> = (props) => {
    ...
    const { user, transactions, onAddTransaction, onUpdateUser, onTransferFunds, isLoading, error, lastTransaction } = props;
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-6">
                <div className="flex space-x-2 border-b border-gray-200 dark:border-gray-700 pb-2">
                    <TabButton active={activeTab === 'new'} onClick={() => setActiveTab('new')}>New Transaction</TabButton>
            ...
            <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 sm:p-8">
                ...
                {activeTab === 'settings' && <SettingsTab user={user} onUpdateUser={onUpdateUser} onTransferFunds={onTransferFunds}/>}
            </div>
        </div>
    );
};
